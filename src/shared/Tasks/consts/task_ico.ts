import { TTaskImg } from "../types/taskImg.enum";
import fBImg from '@shared/Tasks/assets/tasks_img/facebook_ico.svg'
import instImg from '@shared/Tasks/assets/tasks_img/inst_ico.svg'
import tg from '@shared/Tasks/assets/tasks_img/facebook_ico.svg'
import youtube from '@shared/Tasks/assets/tasks_img/facebook_ico.svg'
import twiter from '@shared/Tasks/assets/tasks_img/facebook_ico.svg'
import fire from '@shared/Tasks/assets/tasks_img/Mask group600px_new_fire_ico.svg'





export const  TASKSIMG:TTaskImg={
    instagram:instImg,
    facebook: fBImg,
    telegram:tg,
    twitter:twiter,
    youtube:youtube,
    fire:fire
}